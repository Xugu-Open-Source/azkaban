...
@Singleton
public class XuguDataSource extends AzkabanDataSource {

  public static final int MYSQL_ER_LOCK_DEADLOCK = 1213;

  private static final Logger logger = Logger.getLogger(XuguDataSource.class);
  private final DBMetrics dbMetrics;

  @Inject
  public XuguDataSource(final Props props, final DBMetrics dbMetrics) {
    super();
    this.dbMetrics = dbMetrics;

    final int port = props.getInt("xugu.port");
    final String host = props.getString("xugu.host");
    final String dbName = props.getString("xugu.database");
    ...
    final String url = "jdbc:xugu://" + (host + ":" + port + "/" + dbName);
  ...
  public Connection getConnection() throws SQLException {
    ...
    while (retryAttempt < AzDBUtil.MAX_DB_RETRY_COUNT) {
      try {
         ...
         */
        if (connection == null) {
          throw new SQLException("Failed to find DB connection XUGU. ");
        } else {
